...
export function CompleteScreenBase({ message }: CompleteScreenBaseProps) {
  ...
  const isSavingRecording = recordingStatus === 'uploading'
  const isRecordingSaved = recordingStatus === 'completed'

  return (
    <ScreenLayout>
      <ScreenCard>
        <ScreenIcon variant="success" />
        <ScreenTitle>{t('thankYou.title')}</ScreenTitle>
        <ScreenMessage preserveWhitespace>{message || t('thankYou.defaultMessage')}</ScreenMessage>
        {isSavingRecording && (

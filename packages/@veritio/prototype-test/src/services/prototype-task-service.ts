...
export async function listPrototypeTasks(
  ...
  // Cache the result
  cache.set(cacheKey, tasks as PrototypeTaskWithFrame[], cacheTTL.medium)

  return { data: tasks as PrototypeTaskWithFrame[], error: null }
}
export async function getPrototypeTask(
  ...
  return { data: task as PrototypeTaskWithFrame, error: null }
}
export async function createPrototypeTask(
  ...
  const insertData: PrototypeTestTaskInsert = {
    study_id: studyId,
...
export async function bulkUpdatePrototypeTasks(
  ...
  // Get existing task IDs to detect deletions
  ...
  // Find tasks to delete (exist in DB but not in incoming array)
  ...
  // Handle empty tasks array (all deleted)
  if (tasks.length === 0) {
    invalidatePrototypeTasksCache(studyId)
  ...
  const upsertData: PrototypeTestTaskInsert[] = tasks.map((task) => {
    ...
    // Ensure success_frame_ids is always an array (never null) to satisfy constraint
    const successFrameIds = Array.isArray(task.success_frame_ids)
      ? task.success_frame_ids
      : []

    // For pathway type, ensure success_pathway is a valid object
    // For destination type, success_pathway can be null
    let successPathway = task.success_pathway
    if (criteriaType === 'pathway' && !successPathway) {
      // Default to empty pathway object to satisfy constraint
    ...
    return {
      ...
      state_success_criteria: toJson(task.state_success_criteria || null),
  ...
  // Fetch updated tasks with frame details
  ...
  return { data: updatedTasks as PrototypeTaskWithFrame[], error: null }
}

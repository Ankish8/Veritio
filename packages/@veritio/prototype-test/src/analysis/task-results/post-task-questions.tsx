...
import type { PostTaskQuestion } from '@veritio/prototype-test/lib/supabase/study-flow-types'
...
function formatResponseValue(value: unknown, questionType: string): string {
  ...
  switch (questionType) {
    ...
    default:
      // Text responses (single_line_text, multi_line_text, etc.)
...
export function PostTaskQuestions({
  ...
  // Group and aggregate responses by question
  const aggregatedQuestions = useMemo(() => {
    ...
    // Build aggregated data for each question that has responses
    const result: Array<{
      questionId: string
      questionText: string
      questionType: string
    ...
    // Maintain question order from definitions
            ...
            <CardContent>
              ...
              {q.answers.length > 0 && (
                <div className="space-y-2">
                  {showDistribution ? (
                    // Bar chart distribution for choice-based questions

...
export function usePrototypeTestLogic({
  ...
  const [phase, setPhase] = useState<PrototypeTestPhase>(() => {
    const recordingEnabledInit =
      settings.sessionRecordingSettings?.enabled ?? false;
    if (embeddedMode) {
  ...
  const [questionResponses, setQuestionResponses] = useState<
    TaskQuestionResponses[]
  >([]);
  const [pendingTaskResult, setPendingTaskResult] = useState<TaskResult | null>(
    null,
  );
  ...
  const panelCorner =
    (settings.taskInstructionPosition as PanelCorner) || "top-left";
  ...
  const {
    ...
    buildTaskResult,
    getPathTaken,
  } = usePrototypeTaskTracking();

  // Randomize tasks
  const tasks = useMemo(() => {
    if (!initialTasks || initialTasks.length === 0) return [];
    if (initialTaskId) {
      const startIndex = initialTasks.findIndex(
    ...
    settings.dontRandomizeFirstTask,
  ]);
  ...
  const displayFrameId = useMemo(() => {
    ...
    if (previewMode && !taskStarted) {
      if (currentTask.flow_type !== "free_flow") {
        const successCriteriaType =
          currentTask.success_criteria_type || "destination";
        ...
        if (successCriteriaType === "pathway") {
          ...
          const successFrameIds =
            (currentTask.success_frame_ids as string[]) || [];
  ...
  const currentTaskAttemptId = currentTask
    ? taskAttemptIds[currentTask.id]
    : undefined;
  ...
  const nextPhaseAfterConsent =
    thinkAloudSettings.enabled && thinkAloudSettings.showEducation
  ...
  const submitResults = useCallback(
    async (
      results: TaskResult[],
      allQuestionResponses?: TaskQuestionResponses[],
    ) => {
  ...
  const handleTaskComplete = useCallback(
    (outcome: "success" | "failure" | "skipped") => {
      ...
      const result = buildTaskResult(
        currentTask,
        outcome,
        followedCorrectPathRef.current,
      );

      const questions = castJsonArray<PostTaskQuestion>(
  ...
  const handleStartTask = useCallback(async () => {
    ...
    if (recordingEnabled && recordingConsented && !isRecording) {
      const shouldStartRecording =
        recordingScope === "task" && currentTaskIndex > 0;
  ...
  return {
    ...
    handleRecordingDecline,
    shouldShowRecordingConsent,
    handleThinkAloudEducationComplete,
    handleFigmaLoad,

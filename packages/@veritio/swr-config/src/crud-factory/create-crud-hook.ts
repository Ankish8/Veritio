...
import { getCRUDFetcherRegistry } from './fetcher-registry'
import type {
  CRUDHookConfig,
  CRUDHookReturn,
  CRUDHookOptions,
...
function selectFetcher(type: FetcherType, customFetcher?: <T>(url: string) => Promise<T>) {
  switch (type) {
    ...
    case 'globalUnwrap':
      return getCRUDFetcherRegistry().swrFetcherUnwrap
    case 'public':
      return getCRUDFetcherRegistry().publicFetcher
    case 'custom':
...
>(config: CRUDHookConfig<TData, TItem, TCreateInput, TUpdateInput>) {
  ...
  return function useCRUDHook(
    ...
    const authFetch = getCRUDFetcherRegistry().getAuthFetch()

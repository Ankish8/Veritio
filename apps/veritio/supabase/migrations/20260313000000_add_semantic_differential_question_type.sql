-- Migration: Add semantic_differential question type to study_flow_questions
-- Adds: semantic_differential - bipolar adjective rating scales (e.g., Difficult ↔ Easy)
...
-- Add the new constraint with semantic_differential included
ALTER TABLE study_flow_questions
...
'Question type determines UI and response format. Types: multiple_choice (selection), image_choice (visual grid selection), opinion_scale (discrete scale), yes_no (binary), slider (continuous scale), semantic_differential (bipolar adjectives), nps (0-10), matrix (grid), ranking (reorder). Deprecated: radio, checkbox, dropdown, likert.';

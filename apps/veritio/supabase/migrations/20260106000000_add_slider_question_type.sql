-- Migration: Add slider question type to study_flow_questions
-- Adds: slider - a continuous numeric scale input (e.g., 0-100)
...
-- Add the new constraint with slider included
ALTER TABLE study_flow_questions
...
'Question type determines UI and response format. Types: multiple_choice (selection), opinion_scale (discrete scale), yes_no (binary), slider (continuous scale), nps (0-10), matrix (grid), ranking (reorder). Deprecated: radio, checkbox, dropdown, likert.';

-- Migration: Add constant_sum question type to study_flow_questions
-- Adds: constant_sum - distribute fixed points across items to show relative importance
...
-- Add the new constraint with constant_sum included
ALTER TABLE study_flow_questions
...
CHECK (question_type IN (
  ...
  'constant_sum'            -- Distribute fixed points across items (e.g., 100 points)
));
...
'Question type determines UI and response format. Types: multiple_choice (selection), image_choice (visual grid selection), opinion_scale (discrete scale), yes_no (binary), slider (continuous scale), semantic_differential (bipolar adjectives), constant_sum (point allocation), nps (0-10), matrix (grid), ranking (reorder). Deprecated: radio, checkbox, dropdown, likert.';

...
export class PanelParticipantService {
   ...
   */
  async get(userId: string, participantId: string): Promise<PanelParticipantWithDetails | null> {
    ...
    const { panel_participant_tags: _participantTags, ...rest } = participant
   ...
   */
  async findByEmail(userId: string, email: string): Promise<PanelParticipant | null> {
   ...
   */
  async create(userId: string, input: PanelParticipantInsert): Promise<PanelParticipant> {
    const { data, error } = await this.supabase
      .from('panel_participants')
      .insert({
        ...input,
   ...
   */
  async delete(userId: string, participantId: string): Promise<void> {
   ...
   */
  async bulkDelete(userId: string, participantIds: string[]): Promise<number> {
    const { data, error } = await this.supabase
      .from('panel_participants')
      .delete()
      .eq('user_id', userId)
  ...
  async getOrCreate(
    ...
    const existing = await this.findByEmail(userId, email)
    if (existing) {
      return { participant: existing, created: false }
    }

    // Create new participant
    const participant = await this.create(userId, {
      email,
      ...defaults,
    })
      ...
      if (allTagNames.size > 0) {
        ...
        if (missingTags.length > 0) {
          const { data: newTags } = await this.supabase
            .from('panel_tags')
            .insert(missingTags.map((name) => ({ user_id: userId, name })))
            .select('id, name')
      ...
      try {
        ...
        if (existingId) {
          ...
          if (options.duplicateHandling === 'skip') {
            ...
            // Full update
            await this.update(userId, existingId, {
            ...
            if (Object.keys(updates).length > 0) {
              await this.update(userId, existingId, updates)
            }
            result.updated++
          }
          ...
          // Create new
          const newParticipant = await this.create(userId, {
            email: p.email,
   ...
   * Used for the sidebar badge to show recent signups
   */
  async getRecentSignupCount(userId: string, days: number = 7): Promise<number> {
  ...
  async getBySegmentConditions(
    ...
    // This is a simplified implementation - in production you'd want a more robust query builder
    let query = this.supabase
      .from('panel_participants')
      .select('*', { count: 'exact' })
      .eq('user_id', userId)

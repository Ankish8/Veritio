...
const TEST_CARD_SORT_STUDY_ID = process.env.E2E_CARD_SORT_STUDY_ID;
...
describe("Dashboard - Results & Analysis", () => {
  beforeAll(async () => {
    await login();
    if (TEST_PROJECT_ID && TEST_CARD_SORT_STUDY_ID) {
      open(config.routes.results(TEST_PROJECT_ID, TEST_CARD_SORT_STUDY_ID));
    }
    wait(1000);
  });
...
describe("Dashboard - Results Tabs", () => {
  ...
  it("should show Overview tab", () => {
    ...
    const hasOverviewTab =
      snap.includes("Overview") || snap.includes("Summary");

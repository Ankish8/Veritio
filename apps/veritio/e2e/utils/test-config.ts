...
export const config = {
  ...
  routes: {
    ...
    study: (projectId: string, studyId: string) =>
      `/projects/${projectId}/studies/${studyId}`,
    builder: (projectId: string, studyId: string) =>
      `/projects/${projectId}/studies/${studyId}/builder`,
    results: (projectId: string, studyId: string) =>
      `/projects/${projectId}/studies/${studyId}/results`,
    preview: (projectId: string, studyId: string) =>
      `/projects/${projectId}/studies/${studyId}/preview`,
    recruit: (projectId: string, studyId: string) =>
      `/projects/${projectId}/studies/${studyId}/recruit`,

    // Participant
    player: (studyCode: string) =>
      `/s/${studyCode}${process.env.E2E_PLAYER_PREVIEW === "false" ? "" : "?preview=true"}`,
...
export function getTimeout(
  type: "default" | "navigation" | "action" = "default",
): number {
  switch (type) {
    case "navigation":

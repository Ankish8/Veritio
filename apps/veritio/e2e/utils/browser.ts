...
export function ab(command: string, options: ExecOptions = {}): string {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      const expectedUrl = command.match(/^open "([^"]+)"/)?.[1];

      // Results pages keep long-lived connections open. Playwright can report a
      // load timeout after the navigation itself has already completed.
      if (expectedUrl && /page\.goto: Timeout/i.test(detail)) {
        try {
          const currentUrl = execSync(
            `bunx agent-browser get url ${flags.join(" ")}`.trim(),
            {
              timeout: 10000,
              encoding: "utf-8",
              stdio: ["pipe", "pipe", "pipe"],
          ...
          if (currentUrl === expectedUrl) return currentUrl;
        } catch {
          // Fall through to the normal one-time open retry.
        }
      }

      if (
        /^(back|forward|reload)$/.test(command) &&
        /page\.(goBack|goForward|reload): Timeout/i.test(detail)
      ) {
        try {
          return execSync(
            `bunx agent-browser get url ${flags.join(" ")}`.trim(),
      ...
      const shouldRetryOpen =
        ...
        /EOF|ECONNREFUSED|connection|daemon|page\.goto: Timeout/i.test(detail);
  ...
  throw new Error(`Command failed after retry: ${fullCommand}`);
}

/**
 * Execute command and parse JSON result
 */
export function abJson<T = unknown>(
  command: string,
  options: ExecOptions = {},
): T {
...
export function clickByPlaceholder(
  placeholder: string,
  options?: ExecOptions,
): string {
...
export function fillByPlaceholder(
  placeholder: string,
  text: string,
  options?: ExecOptions,
): string {
...
export function waitForLoad(
  ...
  return ab(`wait --load ${state}`, options);
}
...
export function screenshot(
  path?: string,
  options?: ExecOptions & { fullPage?: boolean },
): string {

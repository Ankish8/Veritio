...
export async function login(
  ...
  while (retries > 0) {
    try {
      ...
      retries--;
      if (retries === 0)
        throw new Error("Failed to open sign-in page after retries");
...
export function resetSession(options: AuthOptions = {}): void {
  ...
  for (const command of [
    ...
    try {
      ab(command, execOptions);
    } catch {
      // The next navigation can initialize a browser that is not running yet.
    }
  }
}

...
export function useUserPreferences() {
    ...
    async (updates: UserPreferencesUpdate): Promise<void> => {
      const optimisticData = data
        ? (deepMerge(
            ...
            updates as DeepPartial<Record<string, unknown>>,
  ...
  const updateAiConfig = useCallback(
    (updates: UserAiConfigUpdate) => updatePreferences({ ai: updates }),
    [updatePreferences],
  );

  const updateWorkspace = useCallback(
    (updates: UserPreferencesUpdate["workspace"]) =>
      updatePreferences({ workspace: updates }),
    [updatePreferences],
  );

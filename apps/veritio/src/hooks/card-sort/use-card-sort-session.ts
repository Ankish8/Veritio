...
export type CardSortPlayerState =
  | "welcome"
  | "recording_consent"
  | "sorting"
  | "submitting"
...
export function useCardSortSession({
  ...
  const [state, setState] = useState<CardSortPlayerState>(
    embeddedMode ? "sorting" : "welcome",
  );
  // In embedded mode, use external session token/participantId; otherwise manage internally
  const [internalSessionToken, setInternalSessionToken] = useState<
    string | null
  >(null);
  const [internalParticipantId, setInternalParticipantId] = useState<
    string | null
  >(null);
  const sessionToken = embeddedMode
    ? (externalSessionToken ?? null)
    : internalSessionToken;
  const participantId = embeddedMode
    ...
    async (
      ...
      try {
        ...
        placedCards.forEach((p) => {
          ...
          categoryAssignments[p.cardId] = p.categoryId;
        });
        ...
        const response = await fetch(
          `/api/participate/${shareCode}/submit/card-sort`,
          {
            method: "POST",
            ...
            body: JSON.stringify({
              ...
              fingerprintConfidence:
                preventionData?.fingerprintConfidence || undefined,
            }),
          },
        );

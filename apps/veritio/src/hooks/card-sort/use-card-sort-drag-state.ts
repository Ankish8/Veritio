...
export function useCardSortDragState({
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (isCategory) {
        if (canPlaceCard && !canPlaceCard(cardId, overId)) {
          onPlacementBlocked?.(overId);
          return;
        }
      ...
      const targetPlacement = placedCards.find((p) => p.cardId === overId);
      if (targetPlacement) {
        if (canPlaceCard && !canPlaceCard(cardId, targetPlacement.categoryId)) {
          onPlacementBlocked?.(targetPlacement.categoryId);
          return;
        }
        ...
        setPlacedCards((prev) => {
          ...
          return [
            ...filtered,
            { cardId, categoryId: targetPlacement.categoryId },
          ];
        });
        return;
      }

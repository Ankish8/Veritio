...
import { DEFAULT_THINK_ALOUD_PROMPTS } from '@/components/builders/shared/types'

export interface UseThinkAloudPromptsOptions {
  /** Whether think-aloud prompts are enabled */
  enabled: boolean
  /** Whether silence has been detected (from useAudioLevelMonitor) */
  isSilent: boolean
  /** Custom prompts array (uses defaults if empty/undefined) */
  customPrompts?: string[]
  /** Minimum seconds between prompts to prevent spam. Default: 30 */
  debounceSeconds?: number
  /** Callback to capture events (from useRecordingEventCapture) */
  captureCustomEvent?: (eventType: string, data: Record<string, unknown>) => void
  /** Current silence duration in seconds (for event data) */
...
export interface UseThinkAloudPromptsReturn {
  ...
  showPrompt: boolean
  /** The current prompt text to display */
  currentPrompt: string
  /** Callback to dismiss the prompt (also rotates to next prompt) */
  dismissPrompt: () => void
  /** Number of prompts shown this session */
  promptCount: number
}
...
/** Manages think-aloud prompts during recording with rotation, debouncing, and event capture. */
export function useThinkAloudPrompts(
  ...
  useEffect(() => {
    ...
    if (timeSinceLastPrompt < debounceSeconds) return

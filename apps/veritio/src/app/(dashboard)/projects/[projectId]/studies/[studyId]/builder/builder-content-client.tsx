...
export function BuilderContentClient({
  ...
  useEffect(() => {
    if (initializedStudyIdRef.current === studyId || !stores.isStoreHydrated)
      return;
    ...
    if (!stores.hasRecoverableMetaDraft) {
      stores.loadMetaFromStudy({
        ...
        file_attachments: ((study.file_attachments as any[] | null) || []).map(
          (f: any) => ({
            ...f,
        ...
        session_recording_settings: (study.session_recording_settings ??
          undefined) as any,
  ...
  const handlePreviewClick = validation.handlePreviewClick;
  const handlePreviewFromBlock = validation.handlePreviewFrom;
  ...
  const previewFromHere = useMemo((): PreviewFromTarget | null => {
    if (activeTab === "study-flow") {
      if (
        urlQuestionId &&
        urlQuestionId !== "intro" &&
        urlQuestionId !== "rejection"
      ) {
        return { kind: "question", id: urlQuestionId };
      }
      return { kind: "section", id: urlSection };
    }

    const activityTabs: BuilderTabId[] = [
      "content",
      "tree",
      "tasks",
      "prototype",
    ...
    return activityTabs.includes(activeTab)
      ? { kind: "section", id: "activity" }
      : null;
  }, [activeTab, urlQuestionId, urlSection]);
  ...
  const refreshStoresFromApiRef = useRef<
    | ((sections?: string[], data?: Record<string, unknown>) => Promise<void>)
    | null
  >(null);

  refreshStoresFromApiRef.current = async (
    sections?: string[],
    ...
    try {
      ...
      const needsContentReload =
        !sections || sections.some((s) => CONTENT_SECTIONS.includes(s));
      const needsQuestionsReload =
        !sections || sections.some((s) => QUESTION_SECTIONS.includes(s));
      const needsSettingsReload =
        !sections || sections.some((s) => SETTINGS_SECTIONS.includes(s));

      // Fast path: settings-only change with data payload — no API calls needed, no shimmer
      if (
        !needsContentReload &&
        !needsQuestionsReload &&
        data?.settings &&
        !data?.study
      ) {
        const payloadSettings = data.settings as Record<string, any>;
        const freshFlowSettings =
          payloadSettings.studyFlow || stores.flowSettings;
      ...
      const needsStudyFetch =
        needsContentReload || needsQuestionsReload || !data?.settings;
      if (needsStudyFetch) {
      ...
      if (needsContentReload) {
        if (studyType === "card_sort") {
          ...
          fetchMap.prototypeTasks = authFetch(
            `/api/studies/${studyId}/prototype-tasks`,
          );
        } else if (studyType === "first_click") {
          fetchMap.firstClick = authFetch(
            `/api/studies/${studyId}/first-click`,
          );
        } else if (studyType === "first_impression") {
          fetchMap.designs = authFetch(
            `/api/studies/${studyId}/first-impression/designs`,
          );
        } else if (studyType === "live_website_test") {
          fetchMap.liveWebsite = authFetch(
            `/api/studies/${studyId}/live-website`,
          );
        }
      }
      ...
      if (res.study?.ok) {
        ...
        rawSettings = (
          freshStudy.settings &&
          typeof freshStudy.settings === "object" &&
          !Array.isArray(freshStudy.settings)
            ? freshStudy.settings
            : {}
      ...
      if (res.flow?.ok && freshStudy) {
        ...
        const freshFlowSettings =
          ...
          migrateToStudyFlowSettings(
            freshStudy.welcome_message,
            freshStudy.thank_you_message,
            undefined,
            freshStudy.study_type,
          );

        stores.loadFlowFromApi({
          flowSettings: freshFlowSettings,
          screeningQuestions: allQuestions.filter(
            (q: any) => q.section === "screening",
          ),
          preStudyQuestions: allQuestions.filter(
            (q: any) => q.section === "pre_study",
          ),
          postStudyQuestions: allQuestions.filter(
            (q: any) => q.section === "post_study",
          ),
          surveyQuestions: allQuestions.filter(
    ...
    const handleAiDataChanged = (e: Event) => {
      ...
      debounceTimer = setTimeout(() => {
        const sectionsToRefresh =
          pendingSections.size > 0 ? Array.from(pendingSections) : undefined;
  ...
  }, [handlePreviewClick, performContentSave]);

  useEffect(() => {
    const handlePreviewFrom = (event: Event) => {
      const target = (event as CustomEvent<PreviewFromTarget>).detail;
      if (
        target &&
        (target.kind === "section" ||
        ...
        handlePreviewFromBlock(target);
      }
    };
    window.addEventListener("builder:preview-from", handlePreviewFrom);
    return () =>
      window.removeEventListener("builder:preview-from", handlePreviewFrom);
  }, [handlePreviewFromBlock]);
  ...
  const propStatus = (study.status || "draft") as
  ...
  return (
    <>
      ...
      <BuilderShell
        ...
        onPreviewFromHere={
          previewFromHere
            ? () => validation.handlePreviewFrom(previewFromHere)
            : undefined
        }

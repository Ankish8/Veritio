    ...
    try {
      ...
      if (isContentDirty) {
        ...
        const invalidCategory = categories.find(
          (category) => !isValidUUID(category.id),
        );
        if (invalidCard || invalidCategory) {
          throw new Error(
            "A card or category has an invalid ID. Please refresh and try again.",
          );
        }
        ...
        const sanitizedCategories = categories.map((cat, index) => ({
          ...
          min_cards: cat.min_cards ?? null,
          max_cards: cat.max_cards ?? null,
        }));

        sentCardSortData = JSON.parse(
          JSON.stringify({ cards, categories, settings }),
        );
        ...
        const extendedSettings = extendSettings(
          settings,
          flowStore,
        ) as ExtendedCardSortSettings;
        savePromises.push(

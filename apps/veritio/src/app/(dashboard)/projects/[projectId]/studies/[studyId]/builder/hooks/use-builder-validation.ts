...
export function useBuilderValidation({
  ...
  const setActiveFlowSection = useStudyFlowBuilderStore(
    (state) => state.setActiveFlowSection,
  );
  const setSelectedQuestionId = useStudyFlowBuilderStore(
    (state) => state.setSelectedQuestionId,
  );
  const setHighlightedItem = useValidationHighlightStore(
    (state) => state.setHighlightedItem,
  );

  const studyType = (study?.study_type || "card_sort") as
    | "card_sort"
    | "tree_test"
    | "survey"
    | "prototype_test"
  ...
  const openPreview = useCallback(
    async (previewFrom?: PreviewFromTarget) => {
      let result;
      try {
        result = validate();
      } catch (error) {
      ...
      if (!result.isValid) {
        ...
        try {
          toast.loading("Saving changes before preview...", {
            id: "preview-save",
          });
        ...
        if (study?.share_code) {
          ...
          if (previewFrom) {
            params.set("previewFrom", serializePreviewFrom(previewFrom));
          }
          const previewUrl = `/s/${study.share_code}?${params.toString()}`;
          const newWindow = window.open(previewUrl, "_blank");
          if (!newWindow) {
            toast.error("Preview blocked by browser", {
              description:
                "Your browser or an extension blocked the preview popup. Please allow popups for this site, then try again.",
              duration: 10000,
            });
          }
        } else {
          toast.error("Unable to preview study", {
            description:
              "Study data could not be loaded. Try refreshing the page or restarting the backend server.",
          });
        }
      }
    },
  ...
  const handlePreviewFrom = useCallback(
    (previewFrom: PreviewFromTarget) => {
      void openPreview(previewFrom);
    },
    [openPreview],
  );
  ...
  const handleConfirmLaunch = useCallback(async () => {
    ...
    try {
      ...
      toast.success("Study launched successfully", {
        description:
          "Participants can now access your study via the share link.",
      });
  ...
  const handleNavigateToIssue = useCallback(
    (issue: ValidationIssue) => {
      const { tab, sectionId, questionId, itemId, itemType, nodeId, taskId } =
        issue.navigationPath;

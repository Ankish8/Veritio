...
import {
  usePanelWidget,
  useWidgetEmbedCode,
} from "@/hooks/panel/use-panel-widget";
...
export function WidgetClient({ studies, organizationId }: WidgetClientProps) {
  const { config, isLoading, updateConfig, setActiveStudy } =
    usePanelWidget(organizationId);
  ...
  const { config: incentiveConfig, updateConfig: updateIncentiveConfig } =
    useStudyIncentiveConfig(activeStudyId);
  ...
  const [extendedSettings, setExtendedSettings] =
    useState<WidgetExtendedSettings>({});

  const remoteConfig =
    (config?.config as PanelWidgetConfigData) || DEFAULT_CONFIG;
  const [localConfig, setLocalConfig] =
    useState<PanelWidgetConfigData>(remoteConfig);
  ...
  const branding: BrandingSettings = useMemo(
    () => ({
      ...
      primaryColor:
        preferences?.studyDefaults?.branding?.primaryColor || "#7c3aed",
      radiusOption:
        preferences?.studyDefaults?.branding?.radiusOption || "default",
    }),
    [preferences],
  );
  ...
  const handleActiveStudyChange = useCallback(
    async (studyId: string | null) => {
      try {
        ...
        toast.success(
          studyId ? "Active study updated" : "Active study cleared",
        );
      } catch {
  ...
  const handleExtendedSettingsChange = useCallback(
    (updates: Partial<WidgetExtendedSettings>) => {
  ...
  if (isLoading) {
    return (
      <>
        <Header title="Widget" />
        <div className="flex min-h-[420px] items-center justify-center p-4 md:h-[calc(100vh-200px)] md:p-6">
    ...
    <>
      <Header title="Widget">
        <div className="grid w-full grid-cols-2 gap-2 md:flex md:w-auto md:items-center md:gap-4">
          <div className="col-span-2 flex h-10 items-center justify-between rounded-md border border-border/60 px-3 md:h-auto md:border-0 md:px-0">
          ...
          <div
            className={cn(
              "flex h-9 items-center gap-1 rounded-md border border-border bg-background p-0.5 shadow-sm",
          ...
          <Button
            ...
            className="h-9 w-full gap-2 md:h-8 md:w-auto"
          >
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (widgetConfig.captureSettings) {
                params.set(
                  "capture-settings",
                  JSON.stringify(widgetConfig.captureSettings),
                );
              }
            ...
            className="col-span-2 h-9 w-full gap-2 md:col-span-1 md:h-8 md:w-auto"
          >
      ...
      <div className="grid gap-4 p-4 md:flex md:h-[calc(100vh-120px)] md:p-6">
        <div
          className={cn(
            "min-w-0",
        ...
        <div className="relative min-h-[560px] flex-1 overflow-hidden rounded-lg border border-border md:min-h-0">
          <InteractivePreview
          ...
          {!widgetConfig.enabled && (
            <div className="absolute inset-0 z-10 flex items-center justify-center bg-gradient-to-b from-black/20 via-black/40 to-black/20 dark:from-black/40 dark:via-black/60 dark:to-black/40 backdrop-blur-[3px]">
              <div className="mx-4 flex max-w-[320px] flex-col items-center gap-4 rounded-2xl bg-background/95 px-6 py-6 text-center shadow-2xl ring-1 ring-border md:px-10 md:py-8">
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <span className="text-xs text-muted-foreground">
                    Turn it on to preview and configure
                  </span>
                </div>

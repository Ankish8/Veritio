...
const CreateSegmentDialog = dynamic(
  () =>
    import("@/components/panel/segments").then((m) => ({
      default: m.CreateSegmentDialog,
    })),
  { ssr: false },
);
...
export function SegmentsClient({ organizationId }: SegmentsClientProps) {
  ...
  const handleEdit = useCallback(
    (segment: PanelSegment, e?: React.MouseEvent) => {
      e?.stopPropagation();
        ...
        {!isLoading && segments.length === 0 && (
          <div className="rounded-xl border border-dashed border-border/60 p-12 text-center bg-card/50">
            ...
            <p className="text-sm text-muted-foreground mt-2 max-w-md mx-auto">
              Create segments to dynamically filter participants based on
              demographics, browser data, location, and activity.
            </p>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem
                              onClick={(e) => handleEdit(segment, e as any)}
                            >
                            ...
                            <DropdownMenuItem
                              onClick={(e) =>
                                handleDuplicate(segment, e as any)
                              }
                            >
                      ...
                      <div className="mt-3 flex flex-wrap items-center gap-2">
                        <Badge variant="secondary" className="font-normal">
                          ...
                          {(segment.conditions?.length || 0) === 1
                            ? "condition"
                            : "conditions"}
                        </Badge>
                        <span className="text-xs text-muted-foreground">
                          {segment.participant_count || 0} participants
                        </span>
                      </div>

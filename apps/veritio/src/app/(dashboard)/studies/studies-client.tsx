...
import {
  StudiesTable,
  type StudyWithCount,
} from "@/components/dashboard/studies-table/studies-table";
import {
  useAllStudies,
  type AllStudiesResponse,
} from "@/hooks/use-all-studies";
...
export function StudiesClient({ initialData }: StudiesClientProps = {}) {
  const { studies, isLoading, isValidating, error, refetch } = useAllStudies(
    {},
    {},
    initialData,
  );
  ...
  const showSkeleton =
    isLoading || (isValidating && studies.length === 0 && !error);
  ...
  const studiesData: StudyWithCount[] = studies.map((study) => ({
    ...
    analysis_included_participant_count:
  ...
  if (error && studies.length === 0) {
    return (
      <div className="flex flex-1 flex-col gap-4 p-4 sm:p-6">
        <div className="rounded-xl border border-dashed border-destructive/30 p-6 text-center bg-destructive/5 sm:p-12">
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 pb-[calc(1rem+env(safe-area-inset-bottom))] sm:p-6">
      <StudiesTable
        studies={studiesData}

...
import type { FlowStep, ResponseValue, StudyFlowResponseInsert } from '@veritio/study-types/study-flow-types'
...
/**
 * Initialize all split stores with study configuration.
 * This is the coordination point for the split stores.
 */
...
/**
 * Reset all split stores to initial state
 */
...
export function usePlayerStoreCompat() {
  ...
  // Shared helpers to avoid repeating the same 5-arg calls
  const getQuestions = () => getQuestionsForStep(
  ...
  const getVisible = () => computeVisibleQuestions(getQuestions(), responses.responses, rules.hiddenCustomSections)
  ...
  return {
    ...
    revealNextQuestion: () => {
      const visibleQuestions = getVisible()
    ...
    // === Computed Values ===
    getCurrentQuestions: getQuestions,

    getVisibleQuestions: getVisible,

    getCurrentQuestion: () => {
      return getVisible()[navigation.currentQuestionIndex] ?? null
    },
    ...
    initializeProgressiveReveal: () => {
      const state = initReveal(getVisible())

...
> {
  ...
  async getOverview(
    ...
    // Run all independent data fetches in parallel — they only depend on studyId
    const [
      smallTablesData,
      [participants, flowResponses],
      largeTablesData,
      { data: flowQuestions },
    ] = await Promise.all([
      this.fetchSmallTables(supabase, studyId),
      Promise.all([
        fetchAllParticipants(supabase, studyId),
        fetchAllFlowResponses(supabase, studyId),
      ]),
      this.fetchLargeTables(supabase, studyId),
      supabase

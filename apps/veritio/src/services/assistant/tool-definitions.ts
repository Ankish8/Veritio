    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          branding: {
            type: 'object',
            description: 'Partial branding settings. Legacy backgroundColor is accepted but inactive; use the nested background contract.',
            properties: {
              primaryColor: { type: 'string', description: 'Six-digit hex color' },
              stylePreset: { type: 'string', enum: ['default', 'vega', 'nova', 'maia', 'lyra', 'mira'] },
              radiusOption: { type: 'string', enum: ['none', 'small', 'default', 'large'] },
              themeMode: { type: 'string', enum: ['light', 'dark', 'system'] },
              background: {
                type: 'object',
                properties: {
                  mode: { type: 'string', enum: ['theme', 'color', 'image'] },
                  color: { type: 'string', description: 'Optional six-digit fallback hex color' },
                  image: {
                    type: 'object',
                    description: 'Only use an already-uploaded, study-owned background asset reference.',
                    properties: {
                      url: { type: 'string' },
                      path: { type: 'string' },
                    ...
                    required: ['url', 'path', 'filename', 'size', 'mimeType'],
                  },
                  layout: { type: 'string', enum: ['fill', 'fit', 'tile'] },
                  position: {
                    type: 'string',
                    enum: [

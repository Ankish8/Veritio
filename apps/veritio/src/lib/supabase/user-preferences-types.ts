...
export interface UserPreferencesRow {
  ...
  favorite_panel_segment_ids: string[] | null;
...
export interface WorkspacePreferences {
  ...
  favoritePanelSegmentIds: string[];
}
...
export function rowToPreferences(
  ...
  return {
    ...
    studyDefaults: {
      branding: {
        primaryColor:
          row.default_primary_color ||
          DEFAULT_STUDY_DEFAULTS_BRANDING.primaryColor,
        backgroundColor:
          row.default_background_color ||
          DEFAULT_STUDY_DEFAULTS_BRANDING.backgroundColor,
        stylePreset:
          row.default_style_preset ||
          DEFAULT_STUDY_DEFAULTS_BRANDING.stylePreset,
        radiusOption:
          row.default_radius_option ||
          DEFAULT_STUDY_DEFAULTS_BRANDING.radiusOption,
        themeMode:
          row.default_theme_mode || DEFAULT_STUDY_DEFAULTS_BRANDING.themeMode,
        logoUrl: row.default_logo_url,
        logoSize:
          row.default_logo_size || DEFAULT_STUDY_DEFAULTS_BRANDING.logoSize,
      },
      settings: {
        language:
          row.default_language || DEFAULT_STUDY_DEFAULTS_SETTINGS.language,
        closingRuleType:
          row.default_closing_rule_type ||
          DEFAULT_STUDY_DEFAULTS_SETTINGS.closingRuleType,
        maxParticipants: row.default_max_participants,
        responsePreventionLevel:
          row.default_response_prevention_level ||
          DEFAULT_STUDY_DEFAULTS_SETTINGS.responsePreventionLevel,
      },
      notifications: {
        enabled:
          row.default_notifications_enabled ??
          DEFAULT_STUDY_DEFAULTS_NOTIFICATIONS.enabled,
        everyResponse:
          row.default_notify_every_response ??
          DEFAULT_STUDY_DEFAULTS_NOTIFICATIONS.everyResponse,
        milestones:
          row.default_notify_milestones ??
          DEFAULT_STUDY_DEFAULTS_NOTIFICATIONS.milestones,
        milestoneValues:
          row.default_milestone_values ||
          DEFAULT_STUDY_DEFAULTS_NOTIFICATIONS.milestoneValues,
        dailyDigest:
          row.default_notify_daily_digest ??
          DEFAULT_STUDY_DEFAULTS_NOTIFICATIONS.dailyDigest,
        onClose:
          row.default_notify_on_close ??
          DEFAULT_STUDY_DEFAULTS_NOTIFICATIONS.onClose,
      },
    },
    dashboard: {
      theme: row.dashboard_theme || DEFAULT_DASHBOARD_PREFERENCES.theme,
      tableDensity:
        row.dashboard_table_density ||
        DEFAULT_DASHBOARD_PREFERENCES.tableDensity,
      showArchived:
        row.dashboard_show_archived ??
        DEFAULT_DASHBOARD_PREFERENCES.showArchived,
    },
    notifications: {
      marketing:
        row.email_marketing ?? DEFAULT_NOTIFICATION_PREFERENCES.marketing,
      productUpdates:
        row.email_product_updates ??
        DEFAULT_NOTIFICATION_PREFERENCES.productUpdates,
      securityAlerts:
        row.email_security_alerts ??
        DEFAULT_NOTIFICATION_PREFERENCES.securityAlerts,
    },
    privacy: {
      analyticsEnabled:
        row.analytics_enabled ?? DEFAULT_PRIVACY_PREFERENCES.analyticsEnabled,
    },
    workspace: {
      lastActiveOrgId: row.last_active_org_id,
      favoritePanelSegmentIds: row.favorite_panel_segment_ids || [],
    },
    onboarding: {
...
export function preferencesToRow(
  ...
  if (prefs.profile !== undefined) {
    if (prefs.profile.avatarUrl !== undefined)
      row.avatar_url = prefs.profile.avatarUrl;
    if (prefs.profile.displayNamePreference !== undefined)
      row.display_name_preference = prefs.profile.displayNamePreference;
  }
  ...
  if (prefs.studyDefaults?.branding !== undefined) {
    ...
    if (b.primaryColor !== undefined)
      row.default_primary_color = b.primaryColor;
    if (b.backgroundColor !== undefined)
      row.default_background_color = b.backgroundColor;
    if (b.stylePreset !== undefined) row.default_style_preset = b.stylePreset;
    if (b.radiusOption !== undefined)
      row.default_radius_option = b.radiusOption;
  ...
  if (prefs.studyDefaults?.settings !== undefined) {
    ...
    if (s.closingRuleType !== undefined)
      row.default_closing_rule_type = s.closingRuleType;
    if (s.maxParticipants !== undefined)
      row.default_max_participants = s.maxParticipants;
    if (s.responsePreventionLevel !== undefined)
      row.default_response_prevention_level = s.responsePreventionLevel;
  }
  ...
  if (prefs.studyDefaults?.notifications !== undefined) {
    ...
    if (n.everyResponse !== undefined)
      row.default_notify_every_response = n.everyResponse;
    if (n.milestones !== undefined)
      row.default_notify_milestones = n.milestones;
    if (n.milestoneValues !== undefined)
      row.default_milestone_values = n.milestoneValues;
    if (n.dailyDigest !== undefined)
      row.default_notify_daily_digest = n.dailyDigest;
  ...
  if (prefs.dashboard !== undefined) {
    ...
    if (d.tableDensity !== undefined)
      row.dashboard_table_density = d.tableDensity;
    if (d.showArchived !== undefined)
      row.dashboard_show_archived = d.showArchived;
  }
  ...
  if (prefs.notifications !== undefined) {
    ...
    if (n.productUpdates !== undefined)
      row.email_product_updates = n.productUpdates;
    if (n.securityAlerts !== undefined)
      row.email_security_alerts = n.securityAlerts;
  }

  // Privacy
  if (prefs.privacy !== undefined) {
    if (prefs.privacy.analyticsEnabled !== undefined)
      row.analytics_enabled = prefs.privacy.analyticsEnabled;
  }

  // Workspace
  if (prefs.workspace !== undefined) {
    if (prefs.workspace.lastActiveOrgId !== undefined)
      row.last_active_org_id = prefs.workspace.lastActiveOrgId;
    if (prefs.workspace.favoritePanelSegmentIds !== undefined) {
      row.favorite_panel_segment_ids = prefs.workspace.favoritePanelSegmentIds;
    }
  }
  ...
  if (aiUpdate !== undefined) {
    if (aiUpdate.openai !== undefined) {
      if (
        aiUpdate.openai.apiKey !== undefined &&
        !isMaskedApiKey(aiUpdate.openai.apiKey)
      ) {
        row.ai_openai_api_key = aiUpdate.openai.apiKey;
      }
      if (aiUpdate.openai.baseUrl !== undefined)
        row.ai_openai_base_url = aiUpdate.openai.baseUrl;
      if (aiUpdate.openai.model !== undefined)
        row.ai_openai_model = aiUpdate.openai.model;
    }
    if (aiUpdate.mercury !== undefined) {
      if (
        aiUpdate.mercury.apiKey !== undefined &&
        !isMaskedApiKey(aiUpdate.mercury.apiKey)
      ) {
        row.ai_mercury_api_key = aiUpdate.mercury.apiKey;
      }
      if (aiUpdate.mercury.baseUrl !== undefined)
        row.ai_mercury_base_url = aiUpdate.mercury.baseUrl;
      if (aiUpdate.mercury.model !== undefined)
        row.ai_mercury_model = aiUpdate.mercury.model;
    }
    if (aiUpdate.useSameProvider !== undefined)
      row.ai_use_same_provider = aiUpdate.useSameProvider;
  }

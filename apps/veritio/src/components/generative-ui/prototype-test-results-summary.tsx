...
interface PrototypeTestResultsSummaryProps {
  totalParticipants?: number
  overallSuccessRate?: number
  averageTime?: number
  tasks?: Array<{ label: string; successRate: number; avgTime?: number; missclickRate?: number }>
...
export function PrototypeTestResultsSummary({
  totalParticipants,
  overallSuccessRate,
  averageTime,
  tasks,
  propStatus,
}: PrototypeTestResultsSummaryProps) {
          ...
          <div className={cn('flex items-center gap-4 flex-wrap', isStreaming && 'animate-pulse')}>
            ...
            {averageTime !== undefined && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-sm font-semibold text-foreground">{formatTime(averageTime)}</span>
                ...
                {tasks.map((task, i) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-foreground truncate max-w-[45%]">{task.label}</span>
                      <div className="flex items-center gap-2">
                        ...
                        {task.missclickRate !== undefined && (
                          <span className="text-[10px] text-muted-foreground">{task.missclickRate}% misclick</span>
                        )}
                      </div>
                    </div>

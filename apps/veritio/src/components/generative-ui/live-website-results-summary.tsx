...
interface LiveWebsiteResultsSummaryProps {
  totalParticipants?: number
  totalSessions?: number
  avgSessionDuration?: number
  tasks?: Array<{ label: string; completionRate?: number; avgTime?: number }>
...
export function LiveWebsiteResultsSummary({
  totalParticipants,
  totalSessions,
  avgSessionDuration,
  tasks,
  propStatus,
}: LiveWebsiteResultsSummaryProps) {
  const isStreaming = propStatus?.tasks === 'streaming'
  const hasData =
    totalParticipants !== undefined || totalSessions !== undefined || (tasks && tasks.length > 0)
          ...
          <div className={cn('flex items-center gap-4 flex-wrap', isStreaming && 'animate-pulse')}>
            ...
            {avgSessionDuration !== undefined && (
              <div className="flex items-center gap-1">
                ...
                <span className="text-sm font-semibold text-foreground">{formatTime(avgSessionDuration)}</span>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        {task.completionRate !== undefined && (
                          <span className={cn('text-xs font-medium', getRateTextColor(task.completionRate))}>
                            {task.completionRate}%
                          </span>
                        )}
                    ...
                    {task.completionRate !== undefined && (
                      <div className="h-1 rounded-full bg-primary/20">
                        <div
                          className={cn('h-full rounded-full transition-all', getRateColor(task.completionRate))}
                          style={{ width: `${Math.min(100, Math.max(0, task.completionRate))}%` }}
                        />
                      </div>
                    )}
                  </div>

...
import type {
  UseCaseDefinition,
  UseCaseCategory,
} from "@/lib/plugins/study-type-icons";
...
export function StudyTypeCard({ useCase }: StudyTypeCardProps) {
  ...
  if (comingSoon) {
    return (
      <div className="relative rounded-xl border border-border/60 p-4 flex flex-col gap-3 w-full opacity-40 cursor-not-allowed">
        <div
  ...
  return (
    <CreateStudyWithProjectDialog
      ...
      trigger={
        <button
          type="button"
          className="group relative flex min-h-36 w-full cursor-pointer flex-col gap-3 rounded-xl border border-border/60 p-4 text-left transition-all duration-150 hover:border-border hover:bg-background/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/50 active:scale-[0.97]"
        >
          <div className="flex items-start justify-between">
            <div
              className={`h-8 w-8 rounded-lg ${bgColor} flex items-center justify-center group-hover:scale-105 transition-transform`}
            >
              <Icon className={`h-4 w-4 ${iconColor}`} />
            </div>

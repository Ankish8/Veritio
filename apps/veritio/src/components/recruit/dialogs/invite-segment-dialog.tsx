...
import { Loader2, Users, AlertTriangle, Filter, Star } from "lucide-react";
...
export function InviteSegmentDialog({
  ...
  const { favoriteIdSet, sortedSegments, toggleFavorite } =
  ...
  const exceedsLimit =
    selectedSegment && selectedSegment.participant_count > MAX_INVITE_LIMIT;

  // Fetch all participant IDs and emails from a segment
  const fetchSegmentParticipants = useCallback(
    async (segmentId: string): Promise<{ ids: string[]; emails: string[] }> => {
      const ids: string[] = [];
      const emails: string[] = [];
      let page = 1;
      const limit = 100;
      ...
      while (hasMore && ids.length < MAX_INVITE_LIMIT) {
        ...
        const data: { data: PanelParticipantWithTags[]; hasMore: boolean } =
          await response.json();
    ...
    try {
      ...
      toast.error(
        error instanceof Error
          ? error.message
          : "Failed to fetch segment participants",
      );
    } finally {
      ...
      <DialogContent className="sm:max-w-md">
            ...
            >
              {sortedSegments.map((segment) => (
                <div key={segment.id} className="flex items-center gap-1">
                  <Label
                    htmlFor={segment.id}
                    className="flex flex-1 items-center gap-3 p-3 rounded-lg border cursor-pointer hover:bg-muted/50 transition-colors [&:has(:checked)]:border-purple-500 [&:has(:checked)]:bg-purple-50/50"
                  >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      void toggleFavorite(segment.id).catch(() =>
        ...
        {exceedsLimit && (
          <div className="flex items-center gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-800 text-sm">
            ...
            <span>
              This segment has {selectedSegment?.participant_count}{" "}
              participants. Only the first {MAX_INVITE_LIMIT} will be invited.
            </span>
          </div>
        )}
        ...
        {selectedSegment && !exceedsLimit && (
          <div className="p-3 rounded-lg bg-muted/50 text-sm">
            <span className="font-medium">
              {selectedSegment.participant_count}
            </span>{" "}
            participant{selectedSegment.participant_count !== 1 ? "s" : ""} will
            be invited
          </div>
        )}

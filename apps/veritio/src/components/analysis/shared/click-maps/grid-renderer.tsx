...
/**
 * Calculate letterbox offset for object-contain images.
 * When an image is displayed with object-contain, it may be centered with padding.
 * This returns the offset needed to map container coordinates to image coordinates.
 */
function calculateLetterboxOffset(
  ...
  if (imageAspect > containerAspect) {
    // Image is wider than container - constrained by width, padding top/bottom
    renderedWidth = containerWidth
    renderedHeight = containerWidth / imageAspect
  } else {
    // Image is taller than container - constrained by height, padding left/right
...
/**
 * Renders a grid overlay on an image showing click distribution.
 *
 * Grid mode visualization:
 * - Divide image into NxN grid cells
 * - Show click count per cell
 * - Color cells with heatmap gradient (blue→cyan→green→amber→red)
 * - Optionally show individual click dots
 */
export const GridRenderer = memo(function GridRenderer({
  ...
  /**
   * Heatmap color ramp: transparent → blue → cyan → green → yellow → red.
   * Anchored to totalClicks so colors stay consistent across grid sizes.
   * A cell with 70% of all clicks looks the same whether in 5x5 or 10x10.
   */
  const getCellColor = (clickCount: number): string => {
    ...
    const stops: [number, number, number, number][] = [
      [59, 130, 246, 0.10],   // blue (low)
      [6, 182, 212, 0.35],    // cyan
      [16, 185, 129, 0.50],   // green
      [245, 158, 11, 0.60],   // amber
      [239, 68, 68, 0.70],    // red (high)
    ]
      ...
      {gridCells.length > 0 && (
        <div className="flex items-center gap-2 mt-2 text-xs text-muted-foreground">
          ...
          <div
            ...
            style={{
              background: 'linear-gradient(to right, rgba(59,130,246,0.15), rgba(6,182,212,0.4), rgba(16,185,129,0.55), rgba(245,158,11,0.65), rgba(239,68,68,0.75))',
            }}
          />

...
import type { FrameWithStats, FrameSortOption } from '@/types/analytics'
...
export const AllPagesGrid = memo(function AllPagesGrid({
  ...
  const sortedFrames = useMemo(() => [...frames]
...
const FrameCard = memo(function FrameCard({ frame, isSelected, onClick }: FrameCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      className={cn(
        'group text-left rounded-lg border transition-all hover:border-muted-foreground/40 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2',

...
export function CardSortPlayer({
  ...
  const [thinkAloudEducationComplete, setThinkAloudEducationComplete] =
    useState(false);
  ...
  const createCategoryWithName = useCallback(
    (name: string): string => {
  ...
  const shuffledPredefinedCategories = useMemo(() => {
    const safeCategories = Array.isArray(initialCategories)
      ? initialCategories
      : [];
  ...
  const canCreateCategory =
    settings.mode === "open" || settings.mode === "hybrid";
  ...
  const canPlaceCard = useCallback(
    (cardId: string, categoryId: string) => {
      const category = shuffledPredefinedCategories.find(
        (candidate) => candidate.id === categoryId,
      );
      return category
        ? canPlaceCardInCategory(placedCards, category, cardId)
        : true;
    },
    [placedCards, shuffledPredefinedCategories],
  );
  ...
  const handlePlacementBlocked = useCallback(
    (categoryId: string) => {
      ...
      if (category?.max_cards == null) return;
      setValidationError(
        `"${category.label}" can contain at most ${category.max_cards} card${
          category.max_cards === 1 ? "" : "s"
        }. Move a card out before adding another.`,
      );
    },
    [setValidationError, shuffledPredefinedCategories],
  );
  ...
  const {
    ...
    placedCards,
    setPlacedCards,
    onCreateUnnamedCategory: canCreateCategory
      ? handleCreateUnnamedCategory
      : undefined,
    canPlaceCard,
  ...
  const handleDragStart = useCallback(
    (event: Parameters<typeof baseDragStart>[0]) => {
  ...
  const handleDragEnd = useCallback(
    (event: Parameters<typeof baseDragEnd>[0]) => {
      const cardId = event.active.id as string;
  ...
  const { getValidationErrors, canSubmit } = useCardSortValidation({
    ...
    customCategories,
    categories: shuffledPredefinedCategories,
    placedCards,
    settings,
  });

  const getCardsInCategory = useCallback(
    (categoryId: string) => {
  ...
  const unnamedCategoriesCount = customCategories.filter(
    (c) => !c.label.trim(),
  ).length;
  const categoryLimitViolations = validateCategoryPlacements(
    shuffledPredefinedCategories,
    placedCards,
  );
  const submitDisabledReason =
    categoryLimitViolations[0]?.message ??
    getSubmitDisabledReason(
  ...
  const needsThinkAloudEducation =
    recordingDecisionMade &&
    thinkAloudSettings.enabled &&
    thinkAloudSettings.showEducation &&
    !thinkAloudEducationComplete;
  if (state === "sorting" && needsThinkAloudEducation) {
    return (
      <ThinkAloudEducationScreen
        onComplete={() => setThinkAloudEducationComplete(true)}
      />
    );
  }
  ...
  return (
    <DesktopSortingView
      ...
      getCategoryCardCount={(categoryId) =>
        getCategoryCardCount(placedCards, categoryId)
      }
      editingCategoryId={editingCategoryId}
      editingCategoryName={editingCategoryName}

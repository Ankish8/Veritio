...
import { getLiveWebsiteConfigurationError, shouldUseCompanionController } from './player-mode'
...
function LiveWebsitePlayerInner({
  ...
  const recordingSettings = useMemo<RecordingSettings>(() => {
    ...
    if (settings.recordScreen && settings.recordWebcam)
      return {
        enabled: true,
        captureMode: mic ? 'video_and_audio' : 'video_only',
        thinkAloud,
      }
    if (settings.recordScreen)
      return {
        enabled: true,
        captureMode: mic ? 'screen_and_audio' : 'screen_only',
        thinkAloud,
      }
  ...
  const [phase, setPhase] = useState<LiveWebsitePhase>(
    configurationError ? 'configuration-error' : recordingSettings.enabled ? 'recording-consent' : 'task-instructions',
  )
  ...
  const handleRecordingConsent = useCallback(
    (preAcquiredStreams?: MediaStream[]) => {
      startRecording(preAcquiredStreams).catch(() => {
  ...
  const websiteUrl = useMemo(() => {
    ...
    const launch = buildLiveWebsiteLaunchUrl({
      ...
      apiOverride:
        typeof window !== 'undefined' && window.location.hostname === 'localhost' ? 'http://localhost:4000' : undefined,
  ...
  const {
    ...
    pipPtqActiveRef,
    closePip,
    handleOpenWebsite: _handleOpenWebsite,
  } = usePipManager({
    currentTask,
    phase,
    getWebsiteUrl,
  ...
  const startTimeLimitTimer = useCallback(() => {
    ...
    timeLimitTimerRef.current = setTimeout(() => {
      ...
      responsesRef.current = [
        ...responsesRef.current,
        buildTaskResponse(currentTask.id, taskStartTimeRef.current, 'timed_out'),
      ]
      advanceToNextTask()
    }, timeLimit * 1000)
  ...
  const pipAdvanceToNextTask = useCallback(
    (postTaskResponses?: Array<{ questionId: string; value: unknown }>) => {
      ...
      responsesRef.current = [
        ...responsesRef.current,
        buildTaskResponse(currentTask.id, taskStartTimeRef.current, 'completed', postTaskResponses),
      ]
      taskStartTimeRef.current = 0
      setInlineConfirmAction(null)
  ...
  const recordResponseAndAdvance = useCallback(
    (postTaskResponses?: Array<{ questionId: string; value: unknown }>) => {
      if (!currentTask) return
      responsesRef.current = [
        ...responsesRef.current,
  ...
  const handlePipContinueWithResponses = useCallback(
    (ptResponses: PostTaskQuestionResponse[]) => {
      ...
      <>
        ...
        {inlineConfirmAction && (
          <InlineConfirmDialog
            ...
            onConfirm={() => {
              const action = inlineConfirmAction
              setInlineConfirmAction(null)
              if (action === 'complete') handleInlineComplete()
              else handleSkipTask()
            }}
    ...
    return (
      <>
        ...
        <SubmittingScreenBase
          message={isUploading ? `Uploading recording... ${uploadProgress}%` : 'Submitting your responses...'}
        />
      </>
    )
  }
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      ...
      >
        ...
        <p
          ...
          style={{
            color: 'var(--style-text-secondary, #64748b)',
            lineHeight: 1.5,
          }}
        >
          {isComplete
